-- =====================================================
-- Data Source Name: CAVG_Flipboard_Views.sql
-- Size: ~600 MB
-- Description: Script to create a cumulative table for Flipboard views
-- Author: MA
...
WITH agg_table1 AS (
  SELECT 
    day, 
    DATETIME_TRUNC(day, WEEK(MONDAY)) AS truncated, -- each week starts on Monday
    republisher_id, 
    republisher_views, 
  FROM 
    `mycompany.daily_republishers_views` -- anonymized table
  WHERE 
    EXISTS (
      SELECT id 
      FROM `mycompany.content` -- anonymized table
      WHERE id = content_id AND region_id = 40 -- edit for the edition you work at
    )
    AND republisher_id = 371 -- filter for Flipboard
    AND day BETWEEN '2024-01-01' AND '2024-12-31' -- date range filter
  ORDER BY 
    day
), 
weekly_table AS (
